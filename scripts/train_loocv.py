#!/usr/bin/env python3
"""
Unified Leave-One-Out Cross-Validation training for Paper 4.

Supports: supervised, n2n, nei2nei, noisier2noise
All methods use REDCNN-SE c=74, identical optimizer/scheduler/EMA.

Usage:
    python train_loocv.py --method supervised --test-patient L506 --device cuda:0
    python train_loocv.py --method n2n --test-patient L067 --device cuda:1
    python train_loocv.py --method nei2nei --test-patient L096 --device cuda:2
"""

import os, sys, time, argparse, logging, random, gc
import numpy as np
import torch
...
from dataset import (compute_metrics, window_normalize, load_ima,
                     _augment_pair)

# ═════════════════════════════════════════════════════════════════════════════
# SHARED CONFIG (identical across all methods)
# ═════════════════════════════════════════════════════════════════════════════
...
BASE_LR    = 2e-4
MIN_LR     = 1e-6
MAX_EPOCHS = 50        # Reduced for LOO-CV (models peak at E14-20)
WARMUP     = 3
VAL_FREQ   = 2
WEIGHT_DECAY = 1e-4
EMA_DECAY  = 0.999
...
GRAD_CLIP  = 0.5
PATCH_SIZE = 128       # Fixed for speed
BATCH_SIZE = 32
PATCHES_PER_SLICE = 8
...
def get_patient_dirs(data_root, kernel):
    """Get QD and FD root directories."""
    base = Path(data_root) / kernel
    if "3mm" in kernel:
        return base / "quarter_3mm", base / "full_3mm"
    elif "1mm" in kernel:
        return base / "quarter_1mm", base / "full_1mm"
    raise ValueError(f"Unknown kernel: {kernel}")


def load_patient_volume(patient_dir, wl=40.0, ww=400.0):
    """Load all .IMA files from a patient directory as a numpy array."""
    imas = sorted(patient_dir.rglob("*.IMA"))
    if not imas:
        return None
...
def load_loocv_data(data_root, kernel, test_patient):
    """Load train/test data for one LOO-CV fold."""
    qd_root, fd_root = get_patient_dirs(data_root, kernel)

    train_qd_vols = []  # list of (N_i, H, W) arrays
    train_fd_vols = []
    test_qd, test_fd = None, None
    ...
    for pdir in sorted(qd_root.iterdir()):
        ...
        if test_patient in pid:
            ...
            train_qd_vols.append(qd_vol)
            train_fd_vols.append(fd_vol)

    return train_qd_vols, train_fd_vols, test_qd, test_fd
...
class N2NDataset(Dataset):
    """Adjacent-slice N2N: input=slice[i], target=slice[i±1] (QD only)."""
...
class NoisierDataset(Dataset):
    """Single-slice dataset for Noisier2Noise (noise added in train loop)."""
    def __init__(self, qd_vols, patch_size=128, pps=8):
        self.data = np.concatenate(qd_vols, axis=0)
        self.ps, self.pps = patch_size, pps
...
def validate(model, val_loader, device):
    """Standard validation: PSNR + SSIM vs FD ground truth."""
    model.eval()
    psnr_list, ssim_list = [], []
...
@torch.no_grad()
def validate_noisier(model, val_loader, device, noise_sigma, sigma_add,
                     n_samples=4):
    """Noisier2Noise corrected validation."""
    model.eval()
    ...
    for qd, fd in val_loader:
        ...
        # Use best of direct and corrected
...
def train_one_fold(method, test_patient, device_str):
    """Train one method on one fold."""
    device = torch.device(device_str)
    fold_dir = os.path.join(SAVE_BASE, f"fold_{test_patient}", method)
    ...
    log.info(f"{'='*60}")
    log.info(f"  LOO-CV: method={method}, test={test_patient}, device={device_str}")
    log.info(f"{'='*60}")

    # ── Load data ──────────────────────────────────────────────────────────
    log.info("Loading data...")
    train_qd, train_fd, test_qd, test_fd = load_loocv_data(
        DATA_ROOT, KERNEL, test_patient)
    ...
    if method == 'supervised':
        train_ds = SupervisedDataset(train_qd, train_fd, PATCH_SIZE, PATCHES_PER_SLICE)
    elif method == 'n2n':
        train_ds = N2NDataset(train_qd, PATCH_SIZE, PATCHES_PER_SLICE)
    elif method == 'nei2nei':
        train_ds = Nei2NeiDataset(train_qd, PATCH_SIZE, PATCHES_PER_SLICE)
    elif method == 'noisier2noise':
        train_ds = NoisierDataset(train_qd, PATCH_SIZE, PATCHES_PER_SLICE)
    else:
    ...
    # Estimate noise sigma for noisier2noise
    noise_sigma, sigma_add = 0.0, 0.0
    if method == 'noisier2noise':
    ...
    log.info(f"  Model: REDCNN_SE c={N_CHANNELS}, {n_params/1e6:.2f}M params")
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if method == 'supervised' or method == 'n2n':
        ...
        if epoch % VAL_FREQ == 0 or epoch == MAX_EPOCHS:
            ...
            if is_best:
                ...
                torch.save({
                    ...
                    'method': method, 'test_patient': test_patient,
    ...
    result = {
        ...
        'n_train_patients': len(ALL_PATIENTS) - 1,
        'log': results_log,
    }
    ...
    log.info(f"\n{'='*60}")
    log.info(f"  DONE: {method} / {test_patient} → best PSNR = {best_psnr:.4f} dB")
    log.info(f"  Saved to {fold_dir}")
    log.info(f"{'='*60}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='LOO-CV training for Paper 4')
    parser.add_argument('--method', required=True,
                        choices=['supervised', 'n2n', 'nei2nei', 'noisier2noise'])
    parser.add_argument('--test-patient', required=True, choices=ALL_PATIENTS)
    parser.add_argument('--device', default='cuda:0')
    args = parser.parse_args()

    train_one_fold(args.method, args.test_patient, args.device)
